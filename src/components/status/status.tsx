...
export default function StatusMain() {
  ...
  useEffect(() => {
    loadMyStatuses()
    loadFollowingStatuses()
  }, [])

  const loadMyStatuses = async () => {
    setLoadingMy(true)
    try {
      const myRes = await axios.get('/api/user/status/getMyStatus')
      setMyStatuses(myRes.data)
    } catch (error) {
      console.error('Error fetching my statuses:', error)
    } finally {
      setLoadingMy(false)
      setLoading(loadingFollowing) // Only set loading false if both are done
    }
  }

  const loadFollowingStatuses = async () => {
    setLoadingFollowing(true)
    try {
      const followingRes = await axios.get('/api/user/status/followingStatus')
      setFollowingStatuses(followingRes.data.groupedStatuses || [])
    } catch (error) {
      console.error('Error fetching following statuses:', error)
    } finally {
      setLoadingFollowing(false)
      setLoading(loadingMy) // Only set loading false if both are done
    }
  }

  if (loadingMy || loadingFollowing) {
