...
export function Button({ title, className, leftIcon, rightIcon, ...rest }: buttonProps) {
  return (
    <motion.button
      ...
      className={`flex w-full items-center justify-center gap-2.5 rounded-[14px] border border-accent bg-accent px-9 py-3.5 text-sm font-semibold text-white duration-100 hover:scale-[0.99] dark:text-black ${className}`}
      {...rest}
    >
      {leftIcon}
      {title}
